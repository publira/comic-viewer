...
import { SpreadsReader } from "./_components/spreads-reader";
...
const SpreadsPage = async () => {
  const source = await readDemoSource(
    "features/spreads/_components/spreads-reader.tsx"
  );
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-5 py-10 text-slate-950 sm:px-8 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        ...
        >
          <SpreadsReader pages={spreadSamplePages} />
        </section>
        <section className="rounded-xl border border-slate-300 bg-white p-5 text-sm leading-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-300">
            ...
            own as a cover and pairs the pages that follow it.
          </p>
          <p className="mt-2 text-slate-600 dark:text-slate-300">
            The alignment this relies on is part of the reader itself. The page
            set, the page slot, and the page each carry{" "}
            <code>data-[page-side=left]</code> and{" "}
            <code>data-[page-side=right]</code> variants, which is what keeps a
            page left unpaired at either end on the half of the spread it
            belongs to.
          </p>
        </section>
