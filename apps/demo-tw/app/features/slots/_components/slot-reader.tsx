...
/**
 * A start or an end page carries the same `data-page-side` attribute as a page,
 * so the same variants keep it on the half of the spread it belongs to.
 */
...
const SlotSheet = ({ children }: PropsWithChildren) => (
  <div className="flex aspect-[4/5] h-full max-w-full items-center justify-center bg-[#19191f] bg-[radial-gradient(120%_80%_at_50%_0%,rgb(255_255_255/0.09),transparent_62%)] p-6 shadow-[inset_0_0_0_1px_rgb(255_255_255/0.08)]">
    <div className="flex w-full max-w-sm flex-col gap-3 rounded-xl border border-slate-100/20 bg-slate-950/80 p-6 text-sm leading-6 shadow-2xl shadow-black/50 backdrop-blur-sm">
      {children}
    </div>
  </div>
);
...
/**
 * A reader that opens on front matter and closes on back matter. A StartPage or
 * an EndPage is composed into the viewer root as a child, a slot holds as many
 * of them as it is written with, and none of them is counted as a page.
