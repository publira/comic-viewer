...
interface LeftToRightReaderProps {
  pages: readonly ViewerPage[];
}

/**
 * A reader that turns pages to the right. Not one utility changes with the
 * direction: the navigation buttons are placed with the logical `start-3` and
 * `end-3`, and the slider fill follows the direction through an `rtl:` variant
 * on a custom property.
 */
export const LeftToRightReader = ({ pages }: LeftToRightReaderProps) => (
  <ComicViewer.Root
